...
zend_class_entry*
uc_iterator_get_ce(void)
{
    return uc_iterator_ce;
}

/* {{{ uc_iterator_item */
static uc_iterator_item_t*
uc_iterator_item_ctor(uc_iterator_t* iterator, uc_cache_slot_t** slot_pp)
{
    zval zvalue;
...
/* {{{ uc_iterator_clone */
static zend_object*
uc_iterator_clone(zval* zobject)
{
...
static void
uc_iterator_item_dtor(uc_iterator_item_t* item)
{
...
/* {{{ uc_iterator_free */
static void
uc_iterator_free(zend_object* object)
{
...
/* {{{ uc_iterator_create */
zend_object*
uc_iterator_create(zend_class_entry* ce)
{
    uc_iterator_t* iterator = (uc_iterator_t*) emalloc(sizeof(uc_iterator_t) + zend_object_properties_size(ce));
 ...
 */
static int
uc_iterator_search_match(uc_iterator_t* iterator, uc_cache_slot_t** slot)
{
    int rval = 1;
...
static int
uc_iterator_check_expiry(uc_cache_t* cache, uc_cache_slot_t** slot, time_t t)
{
    if ((*slot)->value->ttl) {
...
static int
uc_iterator_fetch_active(uc_iterator_t* iterator)
{
    int count = 0;
...
static int
uc_iterator_fetch_deleted(uc_iterator_t* iterator)
{
    int count = 0;
    uc_cache_slot_t** slot;
    uc_iterator_item_t* item;
...
/* {{{ uc_iterator_totals */
static void
uc_iterator_totals(uc_iterator_t* iterator)
{
    uc_cache_slot_t** slot;
    int i;
...
void
uc_iterator_obj_init(uc_iterator_t* iterator, zval* search, zend_long format, zend_long chunk_size, zend_long list)
{
    if (!UCG(enabled)) {
...
#else
        uc_error("Regular expressions support is not enabled, please enable PCRE for " UC_ITERATOR_NAME
                 " regex support.");
#endif
              ...
              PHP_ME(uc_iterator, valid, arginfo_uc_iterator_void, ZEND_ACC_PUBLIC)
                PHP_ME(uc_iterator, getTotalHits, arginfo_uc_iterator_void, ZEND_ACC_PUBLIC)
                  PHP_ME(uc_iterator, getTotalSize, arginfo_uc_iterator_void, ZEND_ACC_PUBLIC)
                    PHP_ME(uc_iterator, getTotalCount, arginfo_uc_iterator_void, ZEND_ACC_PUBLIC) PHP_FE_END
};
/* }}} */

/* {{{ uc_iterator_init */
int
uc_iterator_init(int module_number)
{
    zend_class_entry ce;
